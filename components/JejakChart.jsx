...
import JejakChart from "@/components/JejakChart";
...
export default function JejakForm() {
  ...
  useEffect(() => {
    const saved = localStorage.getItem("jejakHijau");
    if (saved) {
  ...
  const handleFileChange = (e) => {
    setForm((prev) => ({ ...prev, dokumentasi: e.target.files?.[0] || null }));
  };

  const handleSubmit = (e) => {
    e.preventDefault();
    const newData = { ...form, dokumentasi: form.dokumentasi?.name || "" };
    const updated = [...data, newData];
    setData(updated);
    localStorage.setItem("jejakHijau", JSON.stringify(updated));
    setForm({
      nama: "",
      kelas: "",
      kategori: "",
  ...
  const exportPDF = () => {
    ...
    autoTable(doc, {
      head: [["Nama", "Kelas", "Kategori", "Aksi", "Lokasi", "Tanggal", "Poin", "Dokumentasi"]],
      body: data.map((d) => [
        d.nama,
        d.kelas,
        d.kategori,
  ...
  return (
    <div className="bg-white p-6 rounded-lg shadow-md">
      ...
      {data.length > 0 && <JejakChart data={data} />}
    </div>
  );
}
